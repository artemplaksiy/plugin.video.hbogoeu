...
class HbogoHandler_eu(HbogoHandler):
    ...
    def search(self, query=None):
        ...
        if search_text == "":
            ...
            self.log("Performing search: " + self.API_URL_SEARCH + quote(search_text) + '/0/0/0/0/0/3')
            jsonrsp = self.get_from_hbogo(self.API_URL_SEARCH + quote(search_text) + '/0/0/0/0/0/3')
            if jsonrsp is False:
                return
            if self.addon.getSetting('get_elapsed') == 'true':
                self.JsonHis = self.get_from_hbogo(self.API_URL_HIS + self.GOcustomerId + '/' + self.COUNTRY_CODE + '/3', 'json', False)
